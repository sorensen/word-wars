...
function Client(db, socket, attr) {
  this.db = db
  this.socket = socket
...
/**
 * Parse a single client or an object map of clients
 *
 * @param {Object|String} client information
 * @param {String} player id
 * @return {Object|Client} object map or single instance of client
 */

Client.parse = function(data, id) {
  var parsed = {}
  if ('object' === typeof data) {
    for (var id in data) {
      parsed[id] = Client.parse(data[id], id)
    }
    return parsed
  }
...
Client.prototype.defaults = function(attr) {
  var defaults = {
    name: 'anonymous'
  , wins: 0
  , losses: 0
