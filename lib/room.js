...
Room.prototype.saveCurrentPlayers = function(next) {
  ...
  for (var id in this.players) {
    data[id] = this.players[id].toRedis()
  }
...
Room.prototype.loadPlayedWords = function(next) {
...
Room.prototype.join = function(player, next) {
  ...
  if (!this.watchers[player.id]) {
    ...
    this.broadcast('joined', player.toObject())
  }
  next && next()
  return this
}
...
Room.prototype.sit = function(player, next) {
  ...
  this.broadcast('sat', player.id, seat)
  next && next()
  return this
}
